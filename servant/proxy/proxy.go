...
func (this *Proxy) StickyServant(key string) *FunServantPeer {
	checksum := adler32.Checksum([]byte(key)) // adler32 is almost same as crc32, but much 3 times faster
	index := checksum % (len(this.keys) + 1)  // +1 means including me myself
	if index == 0 {
		return nil
	}

	return this.pools[this.keys[index]]
}
